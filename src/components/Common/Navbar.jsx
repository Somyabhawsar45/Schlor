  ...
  return (
    <div className="flex flex-col bg-transparent border-b border-[rgba(6,182,212,0.15)] sticky top-0 z-50 backdrop-blur-sm">
          ...
          <nav className="hidden md:block">
            <ul className="flex gap-x-6">
              {NavbarLinks.map((link, index) => (
                <li key={index}>
                  {link.title === "Catalog" ? (
                    <div className={`group relative flex cursor-pointer items-center gap-1 text-sm transition-colors duration-150 ${matchRoute("/catalog/:catalogName") ? "text-[#06b6d4]" : "text-[#94a3b8] hover:text-[#f0f9ff]"}`}>
                      <p>{link.title}</p>
                      <BsChevronDown />
                    ...
                    <Link to={link?.path}>
                      <p className={`text-sm transition-colors duration-150 ${matchRoute(link?.path) ? "text-[#06b6d4]" : "text-[#94a3b8] hover:text-[#f0f9ff]"}`}>
                        {link.title}
                      </p>
                    </Link>
                  )}
      ...
      {mobileMenuOpen && (
        ...
        >
          <div
            className="absolute right-0 top-0 w-64 bg-[#0c1a2e] border border-[rgba(6,182,212,0.15)] rounded-bl-lg px-6 py-4 flex flex-col gap-4"
          ...
          >
            {NavbarLinks.map((link, index) => (
              <Link
                ...
                to={link?.path || "#"}
                onClick={() => setMobileMenuOpen(false)}
                className={`text-sm transition-colors duration-150 ${matchRoute(link?.path) ? "text-[#06b6d4]" : "text-[#94a3b8] hover:text-[#06b6d4]"}`}
              >
                {link.title}
              </Link>
            ))}
